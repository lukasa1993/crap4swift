...
DEFAULT_TEST_COMMAND = "swift test --enable-code-coverage && swift test --show-codecov-path"
SAFE_GENERATED_NAMES = {"target", "coverage", ".coverage", "build", ".build"}


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="Calculate strict CRAP scores for Swift functions.")
    ...
    value.add_argument("--coverage", type=Path, default=DEFAULT_COVERAGE, help="LCOV, Cobertura, coverage.py, Istanbul, or LLVM JSON report.")
...
def _resolve_swift_report(command_output: str, requested: Path) -> Path:
    ...
    if candidates:
        return candidates[-1]
    raise AnalysisError("SwiftPM did not report an exported coverage JSON path")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if not args.no_test:
            ...
            report = _resolve_swift_report(completed.stdout or "", coverage)
    ...
    except (OSError, ValueError, AnalysisError, json.JSONDecodeError, TimeoutError) as error:
